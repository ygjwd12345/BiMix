...
def compute_mIoU_nighttime(gt_dir, pred_dir, devkit_dir=''):
    """
    Compute IoU given the predicted colorized images and
    """
    with open(join(devkit_dir, 'info.json'), 'r') as fp:
    ...
    image_path_list = join(devkit_dir, 'nighttime_test.txt')
    label_path_list = join(devkit_dir, 'label_nighttime.txt')
    gt_imgs = open(label_path_list, 'r').read().splitlines()
    gt_imgs = [join(gt_dir, x) for x in gt_imgs]
    pred_imgs = open(image_path_list, 'r').read().splitlines()
    pred_imgs = [join(pred_dir, x.split('/')[-1]) for x in pred_imgs]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_dir', default='./dataset/Dark_Zurich', type=str, help='directory which stores CityScapes val gt images')
